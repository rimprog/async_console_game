...
def _apply_acceleration(speed, speed_limit, forward=True):
    ...
    # We move abruptly if the spaceship is stationary
    # Slowly increase speed if the spaceship is already moving
    ...
    # We stop spaceship if speed around zero
...
def update_speed(row_speed, column_speed, rows_direction, columns_direction, row_speed_limit=2, column_speed_limit=2,
    ...
    rows_direction — is a force direction by row axis. Possible values:
       -1 — if force pulls up
       0  — if force has no effect
       1  — if force pulls down

    columns_direction — is a force direction by column axis. Possible values:
    ...
    # We fade speed so that spaceship stops over time
    row_speed *= fading
    column_speed *= fading
